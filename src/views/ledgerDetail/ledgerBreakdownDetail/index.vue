<template>
  <div class="app-container">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="20">
          ...
          <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
              <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" :disabled="disabled"
                v-hasPermi="['ledgerDetail:ledgerBreakdownDetail:add']">新增
              </el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button type="info" plain icon="el-icon-sort" size="mini" @click="toggleExpandAll">收起</el-button>
            </el-col>
            <!-- <el-col :span="1.5">
              <el-button
                type="success"
          ...
          <el-table v-if="refreshTable" v-loading="loading" :data="ledgerBreakdownDetailListCopy" row-key="id" lazy
            :height="'calc(100vh - 205px)'" :header-cell-style="headercellStyle" :cell-style="cellStyle" :load="load"
            ref="table">
            <el-table-column label="标段" fixed="left" align="left" prop="bdbh" min-width="120"
              :show-overflow-tooltip="true" />
            <el-table-column label="子目号" fixed="left" align="left" prop="zmh" min-width="120"
              :show-overflow-tooltip="true">
              <template slot-scope="scope">
                <el-button type="text" size="mini" v-show="!scope.row.children" @click="handleDetails(scope.row)"
                  v-hasPermi="['workflow:process:start']">{{ scope.row.zmh }}</el-button>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="left" fixed="right" class-name="small-padding fixed-width" min-width="200">
              <template slot-scope="scope">
                <el-button size="mini" type="text" icon="el-icon-edit"
                  :disabled="['1', '2'].includes(scope.row.reviewCode)" v-show="!scope.row.children"
                  @click="handleUpdate(scope.row)" v-hasPermi="['ledgerDetail:ledgerBreakdownDetail:edit']">分解数量
                </el-button>
                <!-- <el-button
                  size="mini"
                  type="text"
                ...
                <el-button size="mini" type="text" icon="el-icon-delete"
                  :disabled="['1', '2'].includes(scope.row.reviewCode)" v-show="!scope.row.children"
                  @click="handleDelete(scope.row)" v-hasPermi="['ledgerDetail:ledgerBreakdownDetail:remove']">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog :title="'添加清单'" :visible.sync="openAdd" width="1050px" append-to-body>
      <ledger-list v-if="openAdd" :tzfjbh="queryParams.tzfjbh" :bdbhList="bdbhList" :close="closeOpenAdd"
    ...
    <el-dialog :title="title" :visible.sync="open" width="850px" append-to-body>
      <el-form ref="form" v-if="open" :model="form" :rules="rules" label-width="110px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="分解数量" prop="fjsl" :rules="[
              {
                required: fjslIsRequired,
                message: '分解数量不能为空',
                trigger: 'blur',
              },
            ]">
              <el-input v-model="form.fjsl" placeholder="请输入分解数量" type="number" :min="1" :max="99999999"
                :disabled="!fjslIsRequired" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="变更分解数量" prop="bgfjsl" :rules="[
              {
                required: bgfjslIsRequired,
                message: '变更分解数量不能为空',
                trigger: 'blur',
              },
            ]">
              <el-input v-model="form.bgfjsl" type="number" :min="1" :max="99999999" :disabled="!bgfjslIsRequired"
                placeholder="请输入变更分解数量" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="变更数量" prop="bgsl">
          ...
          <el-col :span="12">
            <el-form-item label="状态">
              <el-radio-group :disabled="true" v-model="form.status">
                <el-radio v-for="dict in dict.type.data_status" :key="dict.value" :label="dict.value">{{ dict.label }}
                </el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>-->
...
export default {
  ...
  computed: {
    //分解数量是否必填
    fjslIsRequired: function () {
      if (this.form.isChange === "1") {
        return false;
      } else if (this.form.isChange === "0") {
        return true;
      } else {
        return false;
      }
  ...
  methods: {
    ...
    getList() {
      ...
      listLedgerBreakdownTree(this.queryParams).then((response) => {
        ...
        this.ledgerBreakdownDetailList = JSON.parse(
    ...
    unload() {
      ...
      this.ledgerBreakdownDetailList = JSON.parse(
        JSON.stringify(this.ledgerBreakdownDetailListCopy)
      ).map((item) => {
    ...
    submitForm() {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          updateLedgerBreakdownDetail(this.form)
            .then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除台账分解明细编号为"' + tzfjbhs + '"的数据项？')
        .then(() => {
          this.loading = true;
    ...
    handleExport() {
      this.download(
        "ledgerDetail/ledgerBreakdownDetail/export",
        {
          ...this.queryParams,
        },
