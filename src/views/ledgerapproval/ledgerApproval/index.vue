    ...
    <el-row :gutter="10">
      <el-col :span="6">
        <div class="left-tree">
          <!-- 新增台账树 -->
          <div style="height: 50%">
            <el-table ref="table" :header-cell-style="headercellStyle" :cell-style="cellStyle" v-loading="qsloading"
              highlight-current-row :data="measurementNoList" @row-click="rowQsClick">
          ...
          <div class="down-tree" style="height: 50%">
            <el-tree :data="ledgerBreakdownList" :props="treeProps" :default-expand-all='true' accordion
      ...
      <el-col :span="18">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
              v-hasPermi="['ledgerapproval:ledgerApproval:add']" :disabled="queryParams.reviewCode == 1 || queryParams.reviewCode == 2
                ">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="submitData"
              v-hasPermi="['ledgerapproval:ledgerApproval:add']" :disabled="queryParams.reviewCode == 1 || queryParams.reviewCode == 2
                ">上报</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
        ...
        <el-table :height="'calc(100vh - 205px)'" v-loading="loading" :header-cell-style="headercellStyle"
          ...
          <el-table-column label="变更数量" align="center" min-width="100" prop="bgsl" :show-overflow-tooltip="true" />
          <el-table-column label="变更分解数量" align="center" min-width="100" prop="bgfjsl" :show-overflow-tooltip="true" />
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" icon="el-icon-delete"
                :disabled="['1', '2'].includes(scope.row.reviewCode)" @click="handleDelete(scope.row, scope.$index)"
                v-hasPermi="['ledgerapproval:ledgerApproval:remove']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-col>
...
import {
  ...
  ledgerApprovalUp,
} from "@/api/ledgerapproval/ledgerApproval";
import {
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listLedgerApproval(this.queryParams)
        .then((response) => {
    ...
    /** 新增按钮操作 */
    async handleAdd() {
      /*  yangaogao  20230803 台账报审新增，不需要判断上期次是否已经审批完，因为在台账期次新增那里已经控制过了。这里是引用期次管理的数据。
    ...
    submitData() {
      ...
      if (["1", "2"].includes(this.queryParams.reviewCode)) {
        ...
        this.$message.warning(
          `选中的当前期次${msg[this.queryParams.reviewCode]}，不可重复上报!`
        );
        return;
      }
      ...
      this.loading = true;
      // 对接上报接口
      ledgerApprovalUp(this.selection)
        .then((response) => {
          this.selection = [];
          this.getList();
          // 上报成功后，刷新左侧申报期次列表
    ...
    handleExport() {
      this.download(
        "ledgerapproval/ledgerApproval/export",
        {
          ...this.queryParams,
        },
    ...
    getLeftTree() {
      ...
      listLedgerBreakdown(params)
        .then((response) => {
          ...
          //lrj #385
          this.ledgerBreakdownList = [{ tzfjmc: 'SG1', id: '0', tzfjbh: '101', tzfjbhParent: '', children: treeData }]
      ...
      listLedgerApprovalNo()
        ...
        .finally(() => {
          if (this.measurementNoList.length) {
            this.rowQsClick(
              this.measurementNoList[this.measurementNoList.length - 1],
              0
            );
            this.currentRow =
              this.measurementNoList[this.measurementNoList.length - 1];
