  ...
  <div class="contract-bill-content">
    <div>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          :disabled="choseContractList.length === 0"
          @click="submitForm"
          >确 定</el-button
        >
...
export default {
  ...
  methods: {
    ...
    findAncestorsName(zmhAncestors) {
      if (!zmhAncestors) return "-";
      const array = zmhAncestors.split(",");
      let str = "";
      array.forEach((item) => {
        str += this.treeData.find((items) => item === items.zmh).zmmc + "/";
      });
    ...
    getList(val) {
      ...
      this.loading = true;
      listContractBill(params)
        .then((response) => {
          this.contractBillList = response.data.map((item) => {
            return {
              ...item,
              zmhAncestorsName: this.findAncestorsName(
                item.zmhAncestors,
                item.zmmc
              ),
            };
      ...
      if (this.choseContractList.length) {
        ...
        if (repeatData.length) {
          this.$message.warning(
            `子目名称为${repeatData.join(" ")}的清单已存在，不可重复添加`
          );
        }
      } else {
